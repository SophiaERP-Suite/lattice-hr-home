...
import { fetchPackageById, initializePackagePayment } from "../../utils/Requests";
import HtmlRenderer from "../../layout/HTMLRenderer";
import { CheckCheck, CheckCircle } from "lucide-react";
import { handlePaymentRequest } from "../../utils/PaymentResponseHandles";
...
const PackageDetailsView = () => {
  ...
  const hashedPackageId = packageId ? Number(hashIds.decode(packageId ?? "")) : 0;
  const hashedId = id ? Number(hashIds.decode(id ?? "")) : 0;
  const navigate = useNavigate();
  
  useEffect(() => {
    fetchPackageById(hashedPackageId)
    .then(res => {
    if (res.status === 200) {
        res.json()
        .then(data => {
  ...
  const submitRequest = async () => {
    ...
    const res = await initializePackagePayment(hashedPackageId, hashedId);
      handlePaymentRequest(res, loader, text, { toast });
  };
  return (
    <>
      <ToastContainer />
        ...
        <div className="container">
            <div className="row">
                ...
                <div className="heading3 text-center space-margin60">
                    ...
                    <h2 style={{ fontSize: '30px' }} data-aos="zoom-in" data-aos-duration="1200">
                          {`
                            ${packageDetails?.currency ?? ""}
                            ${packageDetails?.amount.toLocaleString('en-NG', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) ?? ""}
                            /
                                ...
                                >
                                    <div className="single-pricing-area">
                                        <div className="pricing-box">
                                            <div className="list-heading mt-2">
                                              ...
                                              <ul>
                                                  {
                                                    features.map((data, index) => (
                                                        <li key={index} className="d-flex justify-content-start align-items-center flex-wrap text-wrap gap-2">
                                                            <CheckCircle /> <HtmlRenderer html={data.description} />
                                                        </li>
                                                    ))
                                                  }
                                                </ul>
